...
import "./deildir.scss";
import PDFS from "../../img/PDF/Vefrit -A-Deild";

const deildA = () => (
  <div className="deildir">
    <h1>Vefrit A-Deild</h1>
    <h5>
      Greinar byggðar á rannsóknum höfunda og mega teljast sjálfstæð viðbót við
      núverandi þekkingu.
    </h5>
    {PDFS.map(value => {
      ...
      return (
        <div className="deildir--article" key={value.nr}>
          <div className="deildir--article__info">
            ...
            <span className="deildir--article__info--author">
              {value.author}
            </span>
          ...
          <a className="deildir--article__icons" href={`${value.link}`}>
            <svg>
              <use xlinkHref={`${Icons}#icon-text-document`} />
            </svg>
          </a>
        </div>
      );
